...
if(mouse_dragging_item){
	...
	if(is_instanceof(hovered_item, GridItem) && hovered_item != mouse_pressed_item){
		global.last_operation_result = inventory_try_swap_items(inventory, mouse_pressed_item, inventory, hovered_item, active_slot_x, active_slot_y, mouse_pressed_item_rotation);
	} else { // dragged item to an empty slot or different slot on itself
		var _dest_fit = inventory.can_fit_position(_shape_dragged, active_slot_x, active_slot_y, mouse_pressed_item);
		
		if(_dest_fit){
		    global.last_operation_result = inventory.move_item(mouse_pressed_item, active_slot_x, active_slot_y, mouse_pressed_item_rotation);
		} else {
